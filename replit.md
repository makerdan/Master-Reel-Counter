...
- **Per-Aisle Locking:** Session owners can lock individual aisles via a popover in the session header. Locked aisles prevent non-owner edits to entries, photos, pins, and draft pins in that aisle. Lock enforcement applied on all mutation routes (entries, photos, pins, flag toggle, draft-pins) including destination-aisle checks when moving items. `lockedAisles` table tracks locks; `checkAisleLocked` helper enforces. UI shows lock badges, banners for non-owners, and lock icons throughout PhotoMode and EntryTable.
