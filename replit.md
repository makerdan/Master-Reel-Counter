...
- **Duplicate Detection:** Two detection modes in the Possible Duplicates section of Flagged Reels:
  ...
  - *Same Reel* — pins on the same photo with identical `wireDetails`, suggesting the same physical reel was pinned twice (violet "Same Reel?" badge). Groups by wireDetails string, keyed by sorted pin IDs for disregard persistence.
