...
- **Summary Statistics:** Dashboard for key metrics and activity logs. Enhanced with Data Quality section (flagged pins, flag rate, review response breakdown, dismissed duplicates), AI Scanner Stats (total scans, readable/unreadable rate), Photo Insights (detail shots, regular photos, avg photos per session, photos with linked pins), Wire Breakdown charts (top wire types and gauges with count/footage), Daily Activity Heatmap (last 30 days GitHub-style heatmap), and Session Completion Rate progress bar in Overview.
