...
- 2026-02-22: Bulk operations — batch routes for bulk status update, bulk move to folder, bulk delete; selection checkboxes on session cards; floating bulk action bar with Move/Complete/Reopen/Delete; bulk move dialog for folder selection
- 2026-02-22: Dashboard sorting — sort dropdown with 4 fields (date, name, entries, footage) and toggle direction; sorting applied to folders, unfiled sessions, and shared sessions
- 2026-02-22: Component extraction — session.tsx refactored from 3914 to ~430 lines; 7 components extracted to client/src/pages/session/ directory (ReelCropPreview, TeamDialog, PhotoMode, SingleEntryMode, MobileCaptureView, EntryTable, shared utils.ts and types.ts)
- 2026-02-22: Advanced search — searchUserSessions returns { ownedIds, sharedIds, reasons } with match reasons (name, location, status, date, footage, reels, collaborator, entries); search filters both owned and shared sessions; match reason badges displayed on session cards; recent searches stored in localStorage (max 8) with dropdown on focus, individual remove, clear all
- 2026-02-22: Folder organization system — folders table with sortOrder for custom ordering; create/rename/delete folders; collapsible folder sections on dashboard with session counts; session action dropdown menus (mark complete, edit, duplicate, move to folder, delete); move sessions between folders or to unfiled; duplicate sessions with all entries copied; search bar with "search inside sessions" checkbox filters by session name/location and optionally entry fields (reelTag, wireType, manufacturer, notes, aisle, section)
- 2026-02-21: Receiving auto-increment sections — each Receiving photo gets a sequential section number (001, 002, 003...) in both Photo Mode and Mobile Flow; PDF export shows each photo side-by-side with its pin-linked entries; unmatched entries (no photo) get their own separate page with full-width table when photos also exist in that section
- 2026-02-21: PDF export layout overhaul — first page text data (Location, Status, Entries, Total Footage) now each on separate lines; elapsed time shown on first page directly under session time; section pages restructured with data table first at full width, then associated photos on same/following pages in side-by-side grid (2 per row, single photo centered); section header repeated on continuation pages; position defaults to empty
- 2026-02-20: Single Entry form redesign — larger orange photo buttons, removed Conductors/Wire Type/Wire Size/Color fields, reordered to Aisle→Section→Checkboxes→Category→Vendor Code→Reels→Total Footage→Notes; added "On Floor" and "In Front Of" checkboxes that auto-insert/remove text in Notes; footage field now accepts total footage directly (no longer multiplied by reel count)
- 2026-02-20: Auto-calculating Total Footage — when a catalog category is matched, Total Footage auto-fills with (catalog footage × reel count); user can manually override; override resets when category changes to a new catalog match; uses lastMatchedCatalog ref to avoid re-triggering on same match
- 2026-02-20: Receiving checkbox — auto-fills Aisle="Receiving" and Section="000" when checked, clears both when unchecked; resets on save
- 2026-02-20: PDF export revision — removed full entries table; section photos render at full original aspect ratio with side-by-side layout for detail shots; pin markers changed from filled circles to committed style (transparent rect with orange border, label tab with P#/X# badges, sized per photo's pinScale); section entry table revised to Vendor:/Category:/# of Reels:/Total Footage:/Notes: columns with bold text and underlined headers; Summary Totals headers bold+underlined+colons, category names indented under groups, centered Vendor/Reels/Footage columns, orange reel count per group, Grand Total bold with black outline; Audit Trail labels underlined, CT timestamp format, elapsed time line, Verified Export updated
