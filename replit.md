...
- **Flagged Reels Workflow:** Pins can be flagged for re-shoot/review with an optional reason/note (`flagReason` field). Dedicated "Flagged" tab shows all flagged pins with photo previews, location indicators, flag reasons, and resolve functionality. Flag state and reason persist through draft-pin auto-save and committed pin creation. Reasons can be edited inline in FlaggedReels.
- **Duplicate Detection:** Two detection modes in the Possible Duplicates section of Flagged Reels:
  - *Label Match* — pins with the same label in the same aisle/section (orange "Double Count" or amber "Check Needed" badges).
  - *Same Reel* — pins on the same photo that are within 3× the photo's pin diameter and have matching `wireDetails` (or both empty), suggesting the same physical reel was pinned twice (violet "Same Reel?" badge). Uses union-find clustering on proximity + wire match. Base pin diameter = 4% of image, scaled by photo `pinScale`. Keyed by sorted pin IDs for disregard persistence.
  - Both types support a "Disregard" action (persisted to localStorage) and reuse the `DupPinTile` component. Logic in `client/src/lib/duplicateDetector.ts` (`detectDuplicatePins` + `detectSameReelDuplicates`).
