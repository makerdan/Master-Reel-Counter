#!/usr/bin/env bash
# ci.sh — Sequential CI gate: startup-smoke → audit → typecheck → lint:storage → e2e
...
STEPS=("startup-smoke" "audit" "typecheck" "lint:storage" "e2e")
CMDS=("bash scripts/startup-smoke.sh" "npm audit --audit-level=high" "npm run typecheck" "npm run lint:storage" "node scripts/free-ports.mjs && npm run test:e2e")
