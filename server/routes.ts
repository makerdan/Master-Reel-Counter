...
export async function registerRoutes(
  ...
  const markerDetectionCache = new Map<number, { markers: Array<{ x: number; y: number }> }>();
  ...
  function acquireDetectionLock(sessionId: number, userId: string): { acquired: boolean; lockedBy?: string } {
    ...
    if (existing) {
      if (existing.userId !== userId && (Date.now() - existing.startedAt) < DETECTION_LOCK_TTL) {
        return { acquired: false, lockedBy: existing.userId };
      }
    }
    detectionLocks.set(sessionId, { userId, startedAt: Date.now() });
    return { acquired: true };
  }

  function releaseDetectionLock(sessionId: number) {
    detectionLocks.delete(sessionId);
  }
  ...
  app.post("/api/photos/:photoId/detect-markers", isAuthenticated, async (req: any, res) => {
    try {
      const photoId = parseInt(req.params.photoId);
      const photo = await storage.getPhoto(photoId);
      if (!photo) return res.status(404).json({ message: "Photo not found" });
      ...
      const userId = req.user.claims.sub;
      const lockResult = acquireDetectionLock(photo.sessionId, userId);
      if (!lockResult.acquired) {
        return res.status(409).json({ message: "Detection already in progress", lockedBy: lockResult.lockedBy });
      }
      ...
      try {
        ...
        const maxDim = Math.max(metadata.width || 1500, metadata.height || 1500);
        let downscaled = orientedBuffer;
        if (maxDim > 1500) {
          downscaled = await sharp(orientedBuffer)
            .resize({ width: metadata.width! > metadata.height! ? 1500 : undefined, height: metadata.height! >= metadata.width! ? 1500 : undefined, fit: "inside" })
            .jpeg({ quality: 80 })
            .toBuffer();
        }
        ...
        const response = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "You are analyzing a warehouse photo of wire reels on racks or pallets. Identify all visible bright orange circular sticker dots (approximately 2 inches diameter) placed on or near white paper tags attached to the wire reels. These stickers are intentional markers placed by workers to identify reel locations. For each orange dot found, return its center location as x and y percentages (0-100) of the image dimensions, where (0,0) is the top-left corner and (100,100) is the bottom-right. Be thorough — scan the entire image carefully. Return a JSON object: {\"markers\": [{\"x\": number, \"y\": number}, ...]}. If no orange dots are visible, return {\"markers\": []}.",
            },
            {
              role: "user",
              content: [
                { type: "text", text: "Find all bright orange circular sticker dots in this warehouse photo and return their x,y percentage positions." },
        ...
        const content = response.choices?.[0]?.message?.content ?? "{}";
        let parsed: { markers?: Array<{ x: number; y: number }> } = {};
        try {
          parsed = JSON.parse(content);
        } catch {
          console.error("[detect-markers] Failed to parse OpenAI response:", content);
        }

        const markers = (parsed.markers ?? []).filter(m =>
          typeof m.x === "number" && typeof m.y === "number" &&
          m.x >= 0 && m.x <= 100 && m.y >= 0 && m.y <= 100
        );

        const result = { markers };
        markerDetectionCache.set(photoId, result);
        res.json({ ...result, cached: false });
      } finally {
        releaseDetectionLock(photo.sessionId);
      }
    } catch (error) {
      console.error("Error detecting markers:", error);
      res.status(500).json({ message: "Failed to detect markers" });
    }
  });

  app.get("/api/photos/:photoId/marker-cache", isAuthenticated, async (req: any, res) => {
    try {
      const photoId = parseInt(req.params.photoId);
      const photo = await storage.getPhoto(photoId);
      if (!photo) return res.status(404).json({ message: "Photo not found" });
      ...
      res.status(500).json({ message: "Failed to fetch marker cache" });
    }
  });

  app.get("/api/sessions/:id/detection-lock", isAuthenticated, async (req: any, res) => {
    try {
      const sessionId = parseInt(req.params.id);
      const access = await verifySessionAccess(sessionId, req.user.claims.sub);
      if (!access) return res.status(404).json({ message: "Session not found" });
