...
**How to apply:** Keep `publishableKeyFromHost(window.location.hostname, fallback)` canonical. Use local contract and standard-tier validation for release-harness changes, then use the publish build to verify the exact production origin and production proxy. For synthetic users, prefer Clerk's supported ticket sign-in helper over interactive password flows that can acquire client-trust or MFA steps.