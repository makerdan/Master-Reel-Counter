...
Catalogue discovery is metadata discovery only, not live inference verification.
Ordinary application startup, deployment startup, readiness checks, health
checks, and catalogue refreshes must issue zero model-completion requests.
...
3. Keep insufficiently documented capabilities as `unknown` during ordinary
   operations. Live inference verification requires a separate, explicit,
   protected administrator or operator action.
...
### Bound explicit live inference verification

A single-model verification action must have a fixed per-request timeout and
bounded retry policy. If the application retains bulk verification, restrict it
to models actively used by application route chains, never every catalogue row.
Before issuing requests, set fixed ceilings for model count, concurrency,
attempts per model, per-request timeout, aggregate request count, and the
aggregate operation deadline. These limits must not scale with catalogue size.

When any ceiling is reached, stop launching work and return safe partial or
budget-limited results. Preserve successful evidence already collected, mark
unfinished capabilities `unknown`, and do not convert a budget limit into a
retry fan-out. Protect the action with the application's administrator or
operator authorization, rate limits, audit controls, and safe telemetry.
...
Application startup, deployment startup, readiness checks, ordinary health
checks, and catalogue refreshes must perform zero completion requests. Only a
separate, explicit, protected administrator or operator action may perform live
...
- [ ] Setup performs one catalogue lookup, filters candidates, and probes only
      through a separate protected administrator/operator action; ordinary
      startup, readiness, health, and catalogue refresh perform zero
      completions, and all unverified capabilities remain `unknown`.
- [ ] Bulk live verification, if retained, is limited to active route-chain
      models with fixed model, concurrency, attempt, request-timeout, aggregate
      request, and aggregate deadline ceilings, returning safe partial or
...
- [ ] Setup makes one catalogue request and does not fan out across every model;
      ordinary startup, readiness, health, and catalogue refresh issue zero
      completions.
- [ ] Live inference verification requires an explicit protected
      administrator/operator action and fixed per-model plus aggregate budgets;
      bulk checks cover only active route-chain models and preserve safe partial
      or budget-limited results.
