...
test.describe("session state cleanup @session-cleanup", () => {
  test("trashing a session removes all *-{id} localStorage keys", async ({
    ...
    // Plant scanner keys to simulate a prior scanner session
    await page.evaluate((keys) => {
      const [, zoomKey, selectKey, resultsKey] = keys;
  ...
  test("resetting a session clears scanner/undo keys but preserves session-tab", async ({
    ...
    // Keys cleared by clearSessionResetKeys
    const resetKeys = [
      `scanner-zoom-${sid}`,
      `scanner-select-${sid}`,
      `scanner-results-${sid}`,
